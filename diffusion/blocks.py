...
class GaussianFourierProjection(nn.Module):
    ...
    def forward(self, x):
        ...
        # aw = self.W[None, :]  # [1, 32]
        # awx = ax * aw  # [16, 32]
        h = x[:, None] * self.W[None, :] * 2 * th.pi  # [16, 32]

        h = th.cat([th.sin(h), th.cos(h)], dim=-1)  # [16, 64]
        return h
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if use_cross_attention and cond_dim is not None:
            # Import here to avoid circular import
            from .unet import ConditionCrossAttention
            self.cross_attention = ConditionCrossAttention(channels, cond_dim, num_heads)
            self.cross_norm = normalization(channels)

    def forward(self, x, cond_emb=None):
        if self.use_cross_attention:
            return checkpoint(self._forward, (x, cond_emb), self.parameters(), self.use_checkpoint)
        else:
            return checkpoint(self._forward, (x,), self.parameters(), self.use_checkpoint)

    def _forward(self, x, cond_emb=None):
        b, _, *spatial = x.shape
        ...
        # Cross-attention with conditions (if enabled)
        if self.use_cross_attention and hasattr(self, 'cross_attention') and cond_emb is not None:
            # Apply cross-attention conditioning
            h_cross = self.cross_attention(self.cross_norm(x), cond_emb)
            x = h_cross
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.input_layer = conv_nd(
            dims, in_channels, ch, conv_kernel_size, padding="same"
        )
        ...
        self.output_layer = conv_nd(
            dims, ch, out_channels, conv_kernel_size, padding="same"
        )
